...
export const PROJECT_FORM_SCHEMA = z.object({
  ...
  endDate: z.date({ error: "La fecha de fin debe ser una fecha válida." }).or(z.undefined()),
  total: z.number({ error: "El total debe ser un número." }).min(0, { error: "El total no puede ser negativo." }).or(z.undefined()),
});
